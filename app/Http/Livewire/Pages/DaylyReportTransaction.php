...
class DaylyReportTransaction extends Component
{
    ...
    public function getReportData(){
        $where = $this->setWhere();
        if ($this->startDate){
            return $this->queryReportTicketPerHour([$this->startDate,$this->endDate], $where);
        } else {
            return [];
        }
    }
...
//        $this->validate();
        $this->startDate = $this->input['startDate']??'';
        $this->endDate = $this->input['startDate']??'';
        $this->location = $this->input['location'] ?? 'all';
        $this->vehicle = $this->input['vehicle'] ?? 'all';
        $this->bank = $this->input['bank'] ?? 'all';
        $this->render();
    ...
    public function getResponseData() {
        return  [
            ...
            'title' => 'LAPORAN TRANSAKSI HARIAN TPE',
    ...
    {
        return Excel::download(new \App\Exports\DaylyReportTransaction($this->getResponseData()), 'users.xlsx');
    }
    public function render()
    {
        return view('livewire.pages.dayly-report-transaction', $this->getResponseData());
    }
}
